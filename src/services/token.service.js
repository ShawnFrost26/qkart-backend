...
const generateToken = (userId, expires, type, secret = config.jwt.secret) => {
 const payload = {sub: userId, type: type, exp:expires,iat:Date.now()/1000,};
...
const generateAuthTokens = async (user) => {
  // const token = await generateToken(user._id, Math.floor(Date.now()/1000)+config.jwt.accessExpirationMinutes * 60, tokenTypes.ACCESS, config.jwt.secret);
  // const expires = new Date(Date.now() + config.jwt.accessExpirationMinutes * 60 * 1000);
